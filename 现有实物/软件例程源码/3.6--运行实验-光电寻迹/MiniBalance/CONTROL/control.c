...
{    
	 ...
	 {   
		  ...
		  if(delay_flag==1)
			...
			Get_Angle(Way_Angle);     //===更新姿态		三种之一方法获取角度  yuan
						                                    //5ms读取一次陀螺仪和加速度计的值，更高的采样频率可以改善卡尔曼滤波和互补滤波的效果
			if(Flag_Target==1) 
			{																																							
					return 0;	                                               
			...
			tracking();																																	//读取光电信息，设定小车转向信息。
			
 			Balance_Pwm = balance(Angle_Balance,Gyro_Balance);                  //===平衡PID控制	
	    Velocity_Pwm = velocity(Encoder_Left,Encoder_Right);                  	//===速度环PID控制	 记住，速度反馈是正反馈，
...
{  
    static float Velocity,Encoder_Least,Encoder,Movement=40;
...
{
		static float Turn_Target,Turn,Turn_Convert=1.2;
//	  float Turn_Amplitude=88/Flag_sudu,Kp=42,Kd=0;     
	  float Turn_Amplitude=88/Flag_sudu,Kp=30,Kd=0;     

		if(1==turn_dir)	           			Turn_Target		+=		Turn_Convert;
		else if(2==turn_dir)	     			Turn_Target		-=			Turn_Convert; 
		else Turn_Target=0;
...
//		if(Flag_Qian==1)  Moto1+=DIFFERENCE;   //DIFFERENCE是一个衡量平衡小车电机和机械安装差异的一个变量。
//	  if(Flag_Hou==1)   Moto2-=DIFFERENCE;	  //直接作用于输出，让小车具有更好的一致性。
...
{ 
      ...
      else
      {
					...
					else if(way==3)   Yijielvbo(Accel_Angle,Gyro_X);    			//互补滤波
