	 ...
	 {   
		  ...
		  if(delay_flag==1)
			...
			Get_Angle(Way_Angle);     //===¸üĞÂ×ËÌ¬		ÈıÖÖÖ®Ò»·½·¨»ñÈ¡½Ç¶È  yuan
						                                    //5ms¶ÁÈ¡Ò»´ÎÍÓÂİÒÇºÍ¼ÓËÙ¶È¼ÆµÄÖµ£¬¸ü¸ßµÄ²ÉÑùÆµÂÊ¿ÉÒÔ¸ÄÉÆ¿¨¶ûÂüÂË²¨ºÍ»¥²¹ÂË²¨µÄĞ§¹û
			if(Flag_Target==1) 
			{																																							
					return 0;	                                               
...
//			Read_Distane();                                                      //===»ñÈ¡³¬Éù²¨²âÁ¿¾àÀëÖµ
  		if(Bi_zhang==0)		Led_Flash(100);                          //===LEDÉÁË¸;³£¹æÄ£Ê½ 1s¸Ä±äÒ»´ÎÖ¸Ê¾µÆµÄ×´Ì¬	
			...
			//±ÜÕÏ´¦Àí¸ÄÎªÏòÓÒ
				if(Bi_zhang==1&&Distance<BIZHANG_DIST &&Flag_Qian == 1)			//500
			{			//Ç°·½ÕÏ°­£¬ÏòÓÒ×ª
						Flag_Qian=0,Flag_Hou=0,Flag_Left=0,Flag_Right=1;  //ÓÒ
			}
			else if(Bi_zhang==1&&Distance<BIZHANG_DIST &&Flag_Qian==0&&Flag_Hou==0&&Flag_Left==0&&Flag_Right==0)
			{
						Flag_Hou = 1;
			}
			
			Key();                                                              					//===É¨Ãè°´¼ü×´Ì¬ µ¥»÷Ë«»÷¿ÉÒÔ¸Ä±äĞ¡³µÔËĞĞ×´Ì¬
//			if(click() == 1) 	Flag_Stop	=	!Flag_Stop;								//µ¥»÷¿ØÖÆĞ¡³µµÄÆôÍ£
...
//	  if(Bi_zhang==1&&Distance<BIZHANG_DIST &&Flag_Left!=1&&Flag_Right!=1)     //±ÜÕÏ±êÖ¾Î»ÖÃ1ÇÒ·ÇÒ£¿Ø×ªÍäµÄÊ±ºò£¬½øÈë±ÜÕÏÄ£Ê½
