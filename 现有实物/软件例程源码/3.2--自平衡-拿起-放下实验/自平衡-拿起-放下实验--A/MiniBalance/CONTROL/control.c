 	    ...
 	    Turn_Pwm      = turn(Encoder_Left,Encoder_Right,Gyro_Turn);         //===×ªÏò»·PID¿ØÖÆ     
   		...
   		Xianfu_Pwm();                                                       	//===PWMÏŞ·ù
			...
			if(Put_Down(Angle_Balance,Encoder_Left,Encoder_Right))              //===¼ì²éÊÇ·ñĞ¡³µ±»·ÅÏÂ
					Flag_Stop=0;	                                                      //===Èç¹û±»·ÅÏÂ¾ÍÔÊĞíÆô¶¯µç»ú
...
{ 		   
	 ...
	 if(flag==1)                                                                  //½øÈëµÚ¶ş²½
	 {
	      ...
	      if(Acceleration>16000&&(Angle>(-20+ZHONGZHI))&&(Angle<(20+ZHONGZHI)))   //Ìõ¼ş2£¬Ğ¡³µÊÇÔÚ0¶È¸½½ü±»ÄÃÆğ
		    flag=2; 
	 } 
	 
	 if(flag==2)                                                                  //µÚÈı²½
	 {
	    if(myabs(encoder_left)+myabs(encoder_right)>135)  //Ìõ¼ş3£¬Ğ¡³µµÄÂÖÌ¥ÒòÎªÕı·´À¡´ïµ½×î´óµÄ×ªËÙ   
      {
				flag=0;                                                                                     
